    ...
    def upload_knowledge_document(
        ...
        *,
        ...
        headers = {
            ...
            "X-Agent-Admin-Key": self.settings.oraculo_agent_admin_api_key,
        }
        try:
            with httpx.Client(
                base_url=self.settings.oraculo_agent_base_url,
            ...
            raise GatewayError(
                ...
                code="upstream_unreachable",
                message="No se pudo contactar el servicio remoto.",
                detail={"base_url": self.settings.oraculo_agent_base_url, "error": str(exc)},
            ) from exc
        ...
        if response.status_code >= 400:
            ...
            if response.status_code == 403:
                message = "La llave administrativa del agente no coincide con el deploy remoto."
            raise GatewayError(
                status_code=response.status_code,
                code=str(error_payload.get("code", "upstream_error")),
                message=message,
        ...
        if not isinstance(payload, dict):
            raise GatewayError(
                ...
                code="invalid_upstream_payload",
                message="El servicio remoto devolvio una respuesta invalida.",
                detail={"base_url": self.settings.oraculo_agent_base_url, "path": "/api/v1/knowledge/upload"},
            )
        return payload

    def list_knowledge_sources(self, *, token: str) -> dict[str, Any]:
        return self._request_with_admin(
            method="GET",
            path="/api/v1/knowledge/sources",
            token=token,
        )

    def _request_with_admin(
        self,
        *,
        method: str,
        path: str,
        ...
        if response.status_code >= 400:
            ...
            error_payload = payload.get("error", {}) if isinstance(payload, dict) else {}
            message = str(error_payload.get("message", "El servicio remoto devolvio un error."))
            if response.status_code == 403 and path.startswith("/api/v1/knowledge/"):
                message = "La llave administrativa del agente no coincide con el deploy remoto."
            raise GatewayError(
                status_code=response.status_code,
                code=str(error_payload.get("code", "upstream_error")),
        ...
        if not isinstance(payload, dict):
            raise GatewayError(
                ...
                code="invalid_upstream_payload",
                message="El servicio remoto devolvio una respuesta invalida.",
                detail={"base_url": self.settings.oraculo_agent_base_url, "path": path},
            )
        return payload
